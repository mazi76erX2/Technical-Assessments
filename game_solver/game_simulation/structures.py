...
class SmelterStructure(ProductionStructure, BuildableStructure):
    """
    SMELTER structure for game simulation.

    Converts IRON_ORE into IRON.
    Rate: 2 cycles per PRODUCE action.
    Recipe: 1 IRON_ORE -> 1 IRON per cycle
    """
...
def create_structure_from_api(
    api_structure: ApiStructure, structure_type: StructureType
) -> GameStructure:
    ...
    if structure_type == StructureType.ROAD:
        ...
        if api_structure.storage:
            ...
            for k, v in api_structure.storage.items():
                structure.storage[ResourceType(k)] = v
        return structure
    elif structure_type == StructureType.SMELTER:
